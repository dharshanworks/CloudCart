...
async function seedDatabase() {
  try {
    ...
    const result = await Product.insertMany(
      seedProducts.map(p => ({
        ...p,
        createdBy: new mongoose.Types.ObjectId('000000000000000000000001') // Admin user ID
      }))
    );
    ...
    console.log(`   Average Price: $${(seedProducts.reduce((sum, p) => sum + p.price, 0) / seedProducts.length).toFixed(2)}`);
